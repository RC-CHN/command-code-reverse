...
结论：新增 **Jev / System One 决策 API** 和 **BYOK Responses 协议支持**；现有 `/alpha/generate` 聊天请求及 NDJSON 消费逻辑未发现破坏性变化。经用户授权，用 `.env` 中第一个上游 key 实测 Jev，返回 **HTTP 200**。本次更新分析资料，未修改 Go 代理实现。
...
1. 更新离线 `fallbackVersion`（当前仍为 1.54.2）和上述 8 个模型的静态目录，并从默认展示中移除退役 LongCat 免费条目。动态模型目录仍应优先；客户端传入的新聊天模型 ID 已会原样转发，不受静态表限制。
